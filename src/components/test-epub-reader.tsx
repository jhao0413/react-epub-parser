...
const EpubReader = ({ url }) => {
  ...
  useEffect(() => {
    fetch(url)
      ...
      .then(zip => {
        ...
        if (opfFile) {
          return opfFile.async('string').then(opfContent => ({ opfContent, zip }));
        } else {
      ...
      .then(({ opfContent, zip }) => {
        ...
        return Promise.all([Promise.all(contentPromises), Promise.all(stylePromises), Promise.all(imagePromises)]);
      })
  ...
  return (
    <div>
      <style>{styles}</style>
      <div dangerouslySetInnerHTML={{ __html: replaceImagePaths(content) }}></div>
    </div>
  );
};
